...
DB 복제로 마스터, 슬레이브 구조를 말한다. 

데이터 보호와 부하를 분산처리 할 수 있다. 

데이터 복구 전략으로 다시 한번 MMM vs MHA 
...
주종관계로 한 명이 관리자가 돼서 모든 것을 제어 한다.

리플리케이션의 마스터, 슬레이브에서 사용된다고 볼 수 있다. 

- 오라클의 RAC의 경우 모두가 대등한 Master, Hot-Hot 구조로 대등한 관계이다.  유료...
...
빈도가 높은 문자는 적은 비트의 코드로 변환하고 출현 비도가 낮은 문자는 많은 비트를 사용한다.



### 가역 압축, 비가역 압축

압축한 데이터를 원래 상태로 되돌린다 -> 가역 압축

원래 상태로 복구할수 없다 -> 비가역 압축

사실 Hasing은 비가역 압축이다. 너무 많이 사용한다.
...
데이터 교환을 하고 데이터의 오류를 검출한다.  (체크섬)

jwt로 예를 들자면,

헤더, body를 동일한 알고리즘으로 hash, value를 만들어서 같은 결과가 나온는지 검사한다. 

만약 다른 값이 나오면 변조되었다는 것으로 간주한다. 



### 패리트 비트

홀수, 짝수가 되도록 1비트만 추가하고 비트 검사를 통해 오류를 검출한다. 
